...
class DependanceGenerator(Generator):
	...
	def generate_sample(self):
		...
		for choice in self.choices:
			if choice[1] == self.content_map[self.dependancy_name] or choice[1] == "ANY":
				return choice[0].generate_sample()

		return None
		#raise Exception("Wrong dependance or smth, fuck off")			
...
class MultipleWithoutReplacementGenerator(Generator):
	def __init__(self, name, content_map, gen_count, gen_provider):
		...
		super(MultipleWithoutReplacementGenerator, self).__init__(name, content_map)
...
class TemplateParser():
	...
	def parse_line(self, content_map, line):
		if len(line) > 4:
			...
			return self.parse_statement(var_name, content_map, line.split(':')[1])
	...
	def parse_statement(self,  var_name, content_map, statement):
		...
		for choice_string in choice_strings:
			choice_generators.append(self.parse_choice(var_name, content_map, choice_string))
			'''
			if choice_string[0] == '(' and choice_string[-1] == ')':
				choice_generators.append(self.parse_statement(var_name, content_map, choice_string[1:-1]))
			else:
				choice_generators.append(self.parse_choice(var_name, content_map, choice_string))
			'''
		if conditional == "":
			if isinstance(choice_generators[-1][1], float):
				return ProbabilityGenerator(var_name, content_map, choice_generators)
			else:
				if len(choice_generators) == 1:
					return InterfaceGenerator(var_name, content_map, choice_generators[-1][0])
				else:
					return UniformProbabilityGenerator(var_name, content_map, [x[0] for x in choice_generators])
		else:
			return DependanceGenerator(var_name, content_map, choice_generators, conditional)
	...
	def parse_choice(self, var_name, content_map, choice_string):
		...
		if len(choice_string.split(')')) == 1:
			...
			if choice_string.rfind('-') > choice_string.rfind(')'):
				...
				condition = choice_string[choice_string.rfind('-') + 1:].strip()
			else:
		...
		if value[0] == '(' and value[-1] == ')':
			parsed_down_choice = self.parse_statement(var_name, content_map, value[1:-1])
		else:
			parsed_down_choice = self.parse_literal(var_name, content_map, value)

		if multiplier == 1:
			return parsed_down_choice, condition
		else:
			return MultipleWithoutReplacementGenerator(var_name, content_map, multiplier, parsed_down_choice), condition
...
class AggregateGenerator(Generator):
	def __init__(self, name, content_map, template_file):
		...
		for line in template_file:
			self.generators.append(template_parser.parse_line(content_map, line))
...
def print_sample_aggregate(names, content_map):
	for i in range(len(names)):
		...
		if isinstance(content_map[names[i]], str) or isinstance(content_map[names[i]], unicode):
...
parser = argparse.ArgumentParser(description='Generate object decsriptions by template.')
...
for i in range(repeats):
	...
	if file_type  == "TEMPLATE":
		aggregate_generator = AggregateGenerator("Generator", content_map, template_file)
